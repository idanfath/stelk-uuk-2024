<template>
    ...
    <div>
      ...
      <div class="flex flex-col gap-3 mt-3" v-if="modal.context === 'edit'">
        ...
        <i class="pi pi-pen-to-square"></i>
          ...
          <InputText type="text" v-model="dataItems.find(item => item.id === focusModal).name" placeholder="Customer name" autofocus />
        </InputGroup>
        <InputGroup>
          <InputGroupAddon>
        <i class="pi pi-book"></i>
          </InputGroupAddon>
          <Textarea placeholder="Customer address" v-model="dataItems.find(item => item.id === focusModal).address" rows="2" cols="auto" class="w-full" />
        </InputGroup>
        <InputGroup>
          <InputGroupAddon>
          <i class="pi pi-phone"></i>
      ...
      <div class="flex flex-col gap-3 mt-3" v-if="modal.context === 'view'">
        <InputGroup>
          ...
          <InputText type="text" v-model="dataItems.find(item => item.id === focusModal).name" placeholder="Customer name" readonly />
        </InputGroup>
        <InputGroup>
          <InputGroupAddon>
            <i class="pi pi-book"></i>
          </InputGroupAddon>
          <Textarea placeholder="Customer address" v-model="dataItems.find(item => item.id === focusModal).address" rows="2" cols="auto" class="w-full" readonly />
        </InputGroup>
        <InputGroup>
          <InputGroupAddon>
            <i class="pi pi-phone"></i>
  ...
  <div class="p-5 flex flex-row flex-wrap gap-5" v-motion-fade-visible-once>
    <Panel :toggleable="true" header="Customers" class="w-full">
      <div class="py-5">
        <div class=" flex-col flex gap-2`">
          <h1 class="text-4xl font-bold">Customers</h1>
          <p class="mt-2 text-lg text-gray-500">Here you can add, edit, delete, read your customers. You currently have
        ...
        <DataTable v-model:filters="filters" v-model:selection="selectedItems"
          ...
          <Column field="number" sortable header="Phone">
            <template #body="slotProps">
              <span>{{noUnderScore(slotProps.data.number)}}</span>
            </template>
          </Column>
    ...
    methods: {
      deleteMultiple() {
        this.confirm.require({
          ...
          accept: () => {
            axios.delete('deleteCustomers', { data: { id: this.selectedIds } }).then((response) => {
      ...
      commitDelete(id) {
        axios.delete('deleteCustomer', { data: { id: id } }).then((response) => {
          if (response.status === 200) {
      ...
      onSave() {
        if (this.modal.context === 'add') {
          ...
          form.append('address', this.newItemForm.address);
          form.append('number', this.newItemForm.number);
          console.log(form)
          axios.post('addCustomer', form).then((response) => {
            if (response.status === 200) {
              this.toast.add({ severity: 'success', summary: 'Success', detail: 'Customer has been added', life: 3000 });
              this.fetchMyData();
              this.visible = false;
            }
          }).catch((error) => {
          ...
          form.append('address', this.dataItems.find(item => item.id === this.focusModal).address);
          form.append('number', this.dataItems.find(item => item.id === this.focusModal).number);
          axios.post('editCustomer', form).then((response) => {
            if (response.status === 200) {
              this.toast.add({ severity: 'success', summary: 'Success', detail: 'Customer has been edited', life: 3000 });
              this.visible = false;
            }
          }).catch((error) => {
            this.toast.add({ severity: 'error', summary: 'Error', detail: error.response.data.message, life: 3000 });
      ...
      fetchMyData() {
        ...
        axios.get('fetchAllCustomers').then((response) => {
